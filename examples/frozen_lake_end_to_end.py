...
from torchrl.objectives import DQNLoss, LossModule, ValueEstimators, distance_loss
...
class MctsLossModule(LossModule):
    ...
    def forward(self, tensordict: TensorDictBase) -> TensorDictBase:
        ...
        target_value = tensordict.get("target_value")

        pred_val_index = (pred_val * action).sum(-1)
        pred_val_index = pred_val_index.unsqueeze(0)
...
class Experiment:
    ...
    make_actor_explore: Callable[[EnvBase, QValueActor], TensorDictModuleBase] = None
    ...
    def make_mcts_actor(
        self, env: EnvBase, actor: TensorDictModule
    ...
    def make_model(
        ...
        actor_explore = self.make_actor_explore(env, actor)
    ...
    def make_loss_module(
        ...
        loss_module = MctsLossModule(actor)
